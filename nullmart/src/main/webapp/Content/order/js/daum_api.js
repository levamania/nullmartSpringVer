window.daum = window.daum || {},
    function(a) {
        function b() {
            for (var a = 0, b = l.length; b > a; a++) document.write('<script charset="UTF-8" type="text/javascript" src="' + (i + l[a]) + '"></script>'), document.close();
            h = 2
        }
        ...
        var g = [],
            ...
            k = j[j.length - 1].src;
        j = null;
        var l = ["//t1.daumcdn.net/postcode/api/core/190722/1563776399461/190722.js"];
        if (/\/map_js_init\/postcode.v2(\.dev){0,1}(\.update){0,1}\.js\b/.test(k)) {
            var m = d(k);
            "false" !== m.autoload && (b(), h = 2)
        }
        var n = {};
        a.postcode.version = "190722", a.postcode.load = function(a) {
