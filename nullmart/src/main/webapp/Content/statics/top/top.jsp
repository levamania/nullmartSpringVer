<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<script type="text/javascript" src="/null/Content/statics/top/js/top.js?ver=1"></script>
<link rel="stylesheet" type="text/css" href="/null/Content/statics/top/css/top.css">
<div class="body top1" id="upper_yard">
<jsp:include page="/Content/statics/top/parts/menu.jsp" flush="true"/>
<jsp:include page="/Content/statics/top/parts/search.jsp" flush="true"/>
</div>
<div class="hr" id="hr1"></div>
<div class="body top2">
<jsp:include page="/Content/statics/top/parts/horizentalBar.jsp" flush="true"/>
</div>
<div class="hr" id="hr2"></div>
