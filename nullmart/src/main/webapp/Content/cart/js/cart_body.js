...
function setPrice() {
	...
	$(".content:nth-child(n+2)").each(function () {
		...
		$(this).find(".group").each(function () {
			...
			//배송비 설정
			var deliver = $(this).find(".deliver");
			if (deliver.text() == 'Y') deliver_fee = 2500;
	...
	$("#temp").text(toWon(tot));
	$("#deliver_fee").text(toWon(deliver_fee));
	$("#final").text(toWon(tot + deliver_fee));
...
$().ready(() => {
	...
	$(".revision").on("click", function () {
		...
		if (vacteria.length!=0) {
			//ajax
			$.ajax({
				method: "post",
				url: "/null/cart/update",
				async: false,
