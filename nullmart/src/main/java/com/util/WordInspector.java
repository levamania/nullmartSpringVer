...
public class WordInspector {
	...
	private List<String> censor(String input) throws CustomException {
		...
		if (list == null)
			throw new CustomException("검색가능 단어 없음");
	...
	/**
	 * 입력한 단어를 JSON 형식으로 작성된 파일에 맞추어 번역한다
	 * 
	 * @param input 번역이 필요한 단어
	 * @return 번역된 맵과 번역되지 않은 맵 두개를 리턴 1 * @throws IOException
	 */
	...
	public String render(String raw, Language target_language)
...
//			};		
	...
	public HashMap<String, Object> render(HashMap<String, Object> raw, Language target_language)
			throws JsonParseException, JsonMappingException, IOException {
	...
	/**
	 * 입력된 단어들을 테이블에서 검색가능하도록 카테고리를 붙여주는 작업을 한다. 먼저 테이블에서 각 컬럼명을 가져오고 그 컬럼명으로 컬럼의
	 * 데이터를 가져온다. 입력된 단어와 가져온 데이터가 매칭될 경우 생성한 리스트에 넣고 해당 컬럼의 이름을 키값으로 하여 맵에 저장한다.
	 * 
	 * @param service
	 * @param word_list
	 * @param tables
